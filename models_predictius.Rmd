...
```{r}
lm <- lm(price ~ ., data = cases)
summary(lm)
lm2 <- lm(price ~ area + Overall.Qual + Overall.Cond + Year.Built +
           Garage.Area + Bldg.Type + Central.Air, data = cases)
summary(lm2)
```


2DO: export the data frame created (with only 10 variables) to a CSV file and save the csv file in the GitHub repo. Build the linear model with this data set.